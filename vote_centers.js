...
function initMap() {
  ...
  map.data.addListener('click', (event) => {
    ...
    const sposition = navigator.geolocation.getCurrentPosition();
    const content = `
      <div style="margin-left:20px; margin-bottom:20px;">
        <h2>${name}</h2><p>${address}</p>
        <p><b>Room:</b> ${room}<br/><br/>
        <p><a href="https://maps.google.com?saddr=${sposition.position.coords.latitude}&daddr=${position.lat()},${position.lng()}">Get directions</a>
      </div>
      `;
